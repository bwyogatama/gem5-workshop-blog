...
However, the GPU identity is not visible to the mmap() system call, so ROCk does not know which GPU to map to a doorbell region. We address this issue by encode the GPU ID  into the *offset* parameter passed to mmap(). The encoded offset is returned to user space from the create queue ioctl() ioctl call. Thus, the mmap() system call can decode the GPU ID from the offset parameter, and identify the associated GPU doorbell region used for mapping. This mechanism is illustrated in the Figure 4.
